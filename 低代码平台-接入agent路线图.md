...
让 Agent 从“能用”变成“稳定、可解释、可排查”，并让前端不再只有黑盒转圈等待。
...
- 流式阶段展示
- 前端执行状态流 / 阶段事件推送
- 前端长任务心跳与阶段切换提示，避免黑盒转圈
- diff 预览
- AI 修改分组展示（文案 / 属性 / 事件 / 结构）
- 工具轨迹展示
- 候选节点解释
- 歧义节点澄清闭环
- 结构化执行摘要展示（不直接透传模型原始思维链）
- AI 修改批次的一键回滚入口
- 高风险操作确认（批量删除 / 大范围移动 / 多节点改写）
- patch 范围控制与大范围编辑拦截
- 组件语义模板与动作模板库
- 结果自检与意图一致性检查
- 幂等保障（重复请求不重复插入 / 重复绑定）
...
#### Phase 6.1：模式路由与前端状态流

- 目标：先解决“该走 schema 还是 patch”以及“前端一直转圈不可见”两个最影响体验的问题
- 建议范围：
  - schema / patch 自动模式路由
  - 路由结果可观测，并支持手动覆盖
  - 前端执行状态流 / 阶段事件推送
...
- 本阶段完成标志：
  - 同一入口下可自动区分整页生成与局部编辑
  - 前端可以连续展示至少 3 个阶段状态
  - 任意一次 Agent 请求都能在前端和日志中看到 traceId

#### Phase 6.2：结果预览、澄清与安全护栏

- 目标：让用户在 patch 真正应用前能理解结果，并拦住高风险或歧义操作；同时让同一聊天入口先支持“正常问答”，只有在明确涉及页面结构 / schema / patch 变更时才进入写工具链
- 建议范围：
  - `auto` 模式优先走轻量 LLM 意图分类，而不是直接依赖关键词规则
  - LLM 分类输出固定结构：`mode / confidence / reason / needsPageContext / needsTargetResolution`
  - 先完成意图分类，再决定是否装载页面上下文、是否进入 `schema` / `patch` 写链路
  - 手动模式 `answer / schema / patch` 继续高优先级覆盖，不经过自动分类
  - 规则路由在本阶段仅作为分类失败 / 返回非法结果时的兜底，不在本阶段引入“低置信度自动回退”
  - `answer / schema / patch` 三路自动路由
  - 纯问答与页面理解场景走 `answer` 模式，不要求产出 patch
  - `answer` 模式优先直答；若问题依赖当前页面上下文，可只开放读工具（页面概览 / focus context / candidates / component meta），默认不开放写工具
  - 只有命中“整页生成”或“局部编辑”意图时，才进入 `schema` / `patch` 路径
  - 对“你是谁”“这个页面是做什么的”“为什么这个按钮禁用”这类问题，不再因为有 `pageId/version/selectedId` 就误判为 patch 编辑
  - diff 预览
  - AI 修改分组展示（文案 / 属性 / 事件 / 结构）
  - 候选节点解释
  - 歧义节点澄清闭环
  ...
  - AI 修改批次的一键回滚入口
- 本阶段完成标志：
  - `auto` 模式下，后端会先得到一份结构化 LLM 意图分类结果，再进入正式执行链路
  - 同一聊天入口下，纯问答不再报 “未产出 patch / 未找到目标组件” 这类编辑型错误
  - 问答类请求默认返回自然语言结果，且只有在需要理解页面时才调用读工具
  - 用户能在应用前看懂这次修改改了什么
  - 歧义目标不会直接误改
  - 大范围删除/移动不会静默生效
...
- 目标：减少模型自由发挥导致的偏航，提升简单任务命中率与整体成功率
- 建议范围：
  - 为意图分类增加低置信度回退策略（按需回退到规则路由或保守模式）
  - 建立意图分类评测样本集、误判统计和 prompt 调优闭环
  - 增强反思步骤（结构化 Observe / Reflect）
  - 组件语义模板与动作模板库
  - 结果自检与意图一致性检查
  - 幂等保障（重复请求不重复插入 / 重复绑定）
  - 简单任务快路径
  - 动态步数预算
  - 扩展对话历史窗口
  - 会话级短记忆（后端统一维护会话历史，采用 recent turns + LRU + 摘要压缩）
  - 失败重试与版本冲突恢复策略
  - 请求缓存 / 结果复用策略
- 本阶段完成标志：
  - 高频编辑场景错误率明显下降
  - 简单任务时延明显低于复杂任务
  - 会话内连续多轮编辑时，后端能稳定保留最近上下文而不无限膨胀 token
  - 同指令重复执行不会反复制造重复 patch

#### Phase 6.4：可观测性、回放与评测闭环

- 目标：把 Agent 从“能排障”推进到“能持续优化”
- 补充说明（2026-03-22）：
  当前实现已经具备集合编辑的 `scope_confirmation` 两阶段链路，但对“字段 / 项 / 控件”等业务词汇仍缺少稳定的语义归一化。Phase 6.4 的近期优先项，应把集合编辑链路从“先确认范围”升级为“先确认意思，再确认范围”，同时保持本阶段“可观测性、回放与评测闭环”的主轴不变。
- 建议范围：
  - 工具轨迹展示
  - metrics
  - trace
...
- Should：
  ...
  - 扩展对话历史窗口
- Could：
  - 项目级 / 持久 Memory System
  - 完整 OpenCode 式自由 Agentic Loop

### Phase 6 Backlog 优先级（结合当前实现）

| 建议项                                   | 价值判断   | 范围标记 | 建议优先级 | 建议落点        | 说明                                                                                                           |
| ---------------------------------------- | ---------- | -------- | ---------- | --------------- | -------------------------------------------------------------------------------------------------------------- |
| SSE / 流式阶段输出                       | 高         | Must     | P0         | Phase 6.1       | 直接解决前端黑盒转圈问题，优先返回阶段事件、工具摘要、命中节点、finish reason，不直接透传模型原始思维链        |
| schema / patch 自动模式路由              | 高         | Must     | P0         | Phase 6.1       | 同一入口下自动区分整页生成与局部编辑，是当前闭环继续扩大的前提                                                 |
| Auto 模式 LLM 意图分类                   | 高         | Must     | P0         | Phase 6.2       | `auto` 先让 LLM 判断 `answer/schema/patch` 及是否需要页面上下文，再决定是否进入写链路                          |
| Context Compression                      | 高         | Must     | P0         | Phase 6.1 / 6.3 | 对低代码编辑尤其重要，应优先保证模型看到的是 focus、父子关系、兄弟节点、组件 meta，而不是整页硬塞              |
| 高风险操作确认                           | 高         | Must     | P0         | Phase 6.2       | 适用于批量删除、大范围 move、多节点改写；不建议所有 patch 都强制确认                                           |
| 结果自检 / Self-Correction               | 高         | Must     | P0         | Phase 6.3       | 建议做结构化自检与意图一致性检查，不建议做开放式“再想一遍”的自由反思                                           |
| Tool-Centric 强化                        | 高         | 持续项   | P0         | 持续项          | 这是既定主线，继续围绕 tool registry、guard、template、preview 强化，而不是回退到模型直出 schema               |
| 增强反思步骤（Observe / Reflect）        | 中高       | Must     | P1         | Phase 6.3       | 有价值，但应落成确定性 observe：命中节点、patch 数量、preview 是否通过、是否偏离意图，而不是暴露大段思考文本   |
| 简单任务快路径                           | 中高       | Must     | P1         | Phase 6.3       | 比单纯放宽步数更有效，像“改按钮文案”“加一个 feedback”应尽量少走无效工具回合                                    |
| 动态步数预算                             | 中         | Must     | P1         | Phase 6.3       | 不建议直接把 `maxSteps` 从 6 一刀切到 10；应先做快路径和 stop condition，再按任务类型动态放宽到 8-10           |
| 分类低置信回退与路由评测                 | 中高       | Must     | P1         | Phase 6.3       | 包括低置信度 fallback、误判样本集、命中率统计和分类 prompt 调优，不建议混在 6.2 一次做完                       |
| 扩展对话历史                             | 中         | Should   | P1         | Phase 6.3       | 可从 4 条扩到 6-8 条，但不是当前首要问题；局部编辑更依赖 selectedId、draftSchema、focus context                |
| 会话级短记忆                             | 中         | Should   | P1         | Phase 6.3 / 6.4 | 建议由后端统一维护会话历史，采用 recent turns + LRU + 摘要压缩；先做会话级，不急着做跨项目持久记忆             |
| 集合编辑语义归一化 / intent_confirmation | 中高       | Should   | P1         | Phase 6.4       | 批量修改前的前置语义层，先确认“用户说的是什么”，再进入范围确认；高置信语义可静默继续，多义语义返回业务语言选项 |
| Memory System（项目级设计记忆）          | 中低       | Could    | P2         | Phase 6.4 之后  | 有价值，但必须建立在当前请求稳定、可解释、可回放之后，否则只是把错误长期记住                                   |
| 无用户确认即直接应用                     | 已识别风险 | Must     | P0 风险项  | Phase 6.2       | 需要从“默认直接应用”升级为“按风险等级决定是否确认”                                                             |
| 无持久记忆                               | 已识别问题 | Could    | P2 风险项  | Phase 6.4 之后  | 当前不是最主要瓶颈，先别让持久记忆掩盖命中、稳定性和可观测性问题                                               |
| 完整 OpenCode 式自由 Agentic Loop        | 有参考价值 | Could    | P2         | 长期项          | 当前更需要“可控执行的编辑器 Agent”，不是更自由的通用型长链推理 Agent                                           |

### 当前不建议直接照搬的点

- 不建议直接向前端透传模型原始思考过程，应改为结构化阶段事件和执行摘要
- 不建议所有 patch 都加确认，否则会显著破坏编辑流畅性；只对高风险 patch 加确认
- 不建议把“步数不够”当成当前问题的主要根因，先排查无效工具回合、上下文冗余和结束条件不清晰
- 不建议在当前阶段优先做跨项目持久记忆，先把当前请求闭环做稳

### 验收标准

- [ ] 同一聊天入口下，整页生成类指令能自动命中 schema 模式，局部编辑类指令能自动命中 patch 模式
- [ ] 自动模式路由结果可被日志与前端调试信息观测，必要时允许手动覆盖
- [ ] 用户可以看到当前处理阶段，而不是纯黑盒等待
- [ ] 前端在较慢请求中至少能展示“解析目标 / 读取上下文 / 调用工具 / 校验 patch / 完成”等阶段状态，而不是只显示 loading
- [ ] 前端展示的是结构化执行状态、工具摘要和命中结果，不直接暴露模型原始思维链
- [ ] patch 应用前可预览 diff
- [ ] AI 返回的 patch 能按“文案 / 属性 / 事件 / 结构”分组展示，便于用户快速理解改动
- [ ] 至少能查看工具调用轨迹和最终命中节点
- [ ] 候选节点不明确时，前端能拿到候选解释并支持澄清或重新选择
- [ ] 高风险操作不会静默落地，至少有确认或策略拦截
- [ ] patch 超过范围限制时能被拦截，并给出明确原因
- [ ] feedback / apiCall / dialog / navigate 等高频动作有稳定模板，不再频繁产出错字段
- [ ] Agent 产出 patch 后至少经过一次结果自检，能拦截明显不符合用户意图的结果
- [ ] 相同目标和相同指令重复执行时，不会重复插入组件或重复绑定同一批动作
- [ ] 简单任务存在快路径，平均耗时明显低于复杂多步任务
- [ ] 复杂任务可以获得比简单任务更高的步骤预算，但不会无限放宽
- [ ] 会话级短记忆由后端统一维护，并采用 recent turns + LRU + 摘要压缩，避免长对话直接堆满上下文
- [ ] 版本冲突、目标歧义、工具失败等错误能区分类型并给出恢复路径
...
- [ ] 路由验证：发送“生成一个登录页”命中 schema 模式，发送“把这个按钮改成提交”命中 patch 模式
- [ ] 状态流验证：构造一次耗时较长的 Agent 请求，前端能连续收到并展示至少 3 个阶段事件
- [ ] 澄清验证：构造一个“那个按钮”类歧义指令，前端能看到候选解释并完成一次选择
- [ ] 安全验证：构造一次批量删除或大范围移动，系统会要求确认或直接拦截
- [ ] 幂等验证：对同一页面、同一目标、同一指令连续执行 2 次，不会出现重复插入或重复绑定
- [ ] 快路径验证：对“改按钮文案”与“重做页面结构”分别计时，简单任务明显更快
- [ ] 步数预算验证：简单任务与复杂任务命中不同策略，日志中可看到动态步数决策
- [ ] 会话短记忆验证：同一会话连续多轮编辑时，后续请求能复用最近澄清结果或上下文
- [ ] 历史压缩验证：长会话超过阈值后，后端会保留 recent turns + summary，而不是无限堆积原始历史
...
| Phase 6 | 增强体验与可观测性                     | Agent 闭环、trace 基础                   | 模式自动路由、状态流、diff、澄清、自检、轨迹、metrics、replay、评测集 | 用户能看到阶段、diff、trace 与路由结果 | 评测样本回归测试                               | Playwright 可做 + 手动复盘 | 观测数据采集、回放结构设计           | 1-2 周              |
...
| Phase 6 | 你       | Playwright 验路由/diff/stream，手动做一次失败排障复盘 |
