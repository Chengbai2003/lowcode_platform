...
export function buildSystemPrompt(componentList: readonly string[]): string {
  ...
  return [
    ...
    '如需修改页面逻辑（State、Computed 或 ActionFlow），必须先用 get_page_schema 读取当前 schema.logic，再调用 replace_page_logic 提交完整 logic 对象，保留未要求修改的声明。创建 flow 后才可用 bind_event 绑定 runFlow；删除或改名 flow 前先重新绑定其引用。',
    '如果你已经完成修改，就停止继续调用工具。',
  ].join('\n');
}
...
export function buildPrompt(
  ...
  chunks.push(
    [
      ...
      '4. 删除或移动前确认目标组件明确。',
      '5. 修改页面逻辑（State、Computed 或 ActionFlow）必须先调用 get_page_schema 读取当前值，再用 replace_page_logic 提交完整 logic，保留未要求修改的声明；先创建 flow 再用 bind_event 绑定 runFlow，删除或改名前先重新绑定引用。',
    ].join('\n'),
  );
  return chunks.join('\n\n');
}
