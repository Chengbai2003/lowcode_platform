...
Respond ONLY with valid JSON. Do not include markdown formatting or explanations.`;
  ...
  async chat(dto: ChatRequestDto) {
    const { model } = this.providerFactory.resolveModel(
      dto.modelId,
      dto.provider,
    );

    this.logger.log(
      `[chat] Using model: ${dto.modelId || dto.provider || "default"}`,
    );
  ...
  chatStream(dto: ChatRequestDto): any {
    ...
    this.logger.log(
      `[chatStream] Using model: ${dto.modelId || dto.provider || "default"}`,
    );
  ...
  async generateSchema(dto: GenerateSchemaDto) {
    ...
    const messages: any[] = [
      { role: "system", content: this.codeGenSystemPrompt },
    ];
