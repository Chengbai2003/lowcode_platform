...
export class MockAIService implements AIService {
  ...
  async generateResponse(request: AIRequest): Promise<AIResponse> {
    ...
    if (prompt.includes("表格") || prompt.includes("table")) {
      return {
        content:
          "我创建了一个数据表格，包含姓名、年龄、地址三列。你可以通过设置dataSource属性来填充实际数据。",
        schema: {
          rootId: "root",
          components: {
            root: {
    ...
    if (
      ...
      return {
        content:
          "我为你设计了一个现代化的导航栏，包含Logo、菜单项和用户操作区域。",
        schema: {
          rootId: "root",
          components: {
            root: {
    ...
    return {
      content:
        '我为你创建了一个基础容器结构。你可以继续告诉我需要添加什么具体内容，比如"添加一个按钮"、"生成一个表单"等。',
      schema: {
        rootId: "root",
        components: {
          root: {
  ...
  async analyzeSchema(
    ...
    const hasForm = Object.values(schema.components).some(
      (comp) => comp.type === "Form",
    );
    const hasTable = Object.values(schema.components).some(
