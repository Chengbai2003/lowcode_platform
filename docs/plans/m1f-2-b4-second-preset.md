...
| 别名写入规范化与复杂 Patch 全链路闭环：覆盖单别名插入、别名插入后删除、同 ID 覆盖/异构别名重建；经真实工具出口（preview_patch/write tools）→ 前端真实 applyPatchToSchema 重放 → CAS 保存 → Compiler 导出全链路闭环 | `b4-second-preset.integration.spec.ts` S3（3 个完整闭环用例）+ `b4-frontend-second-preset.test.tsx`（3 个重放用例）+ `patch-validation.service.spec.ts`（42 用例） |
...
| 前端真实 Catalog 双 Preset 并存、精确解析、版本不匹配 fail-close                                                                                                                                                   | `b4-frontend-second-preset.test.tsx`（7 核心用例，全文件共 10 用例）                                                                                               |
...
- **别名写入规范化与复杂 Patch 全链路闭环（阶段 0 收口 / 约束 4）**：`PatchValidationService`
  ...
  已由测试完整证明全链路闭环：真实工具出口（`preview_patch` 与 write tools）返回规范化 Patch → 前端真实 `applyPatchToSchema` 重放验证与预览 Schema 严格一致 → 真实 CAS 保存校验 `basePageVersion` 成功 → 真实 Compiler 编译成功且产物干净。绕过 Agent 入口手写的别名 Schema（直接 `PUT` 原始 JSON）
