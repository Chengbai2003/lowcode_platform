...
M0-4 的内置 Renderer/Preset/RuntimeSession 已完成：Renderer 可被最小 React 宿主仅依赖 Contract + Renderer + 一个 Preset 渲染 JSON；Registry 与 canonical Schema 无法被运行时组件修改；非法 Props 在渲染前被拒绝。外部组合 Preset 若需要后端编译，仍必须由服务端部署并注册对应可信 binding；客户端声明不会成为可信导入来源。
