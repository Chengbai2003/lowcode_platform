...
| M0-3 Agent Eval      | P0     | 确定性 Patch Apply 基线可运行；完整生产编排与 Live 趋势契约仍待完成 | In Progress |
| M0-4 独立 Renderer   | P0     | 内置 Preset/Session 完成；外部 Preset 服务端可信注册仍待产品化      | In Progress |
...
5. `[M1a-1] [Logic] 引入 logic.states 与安全 computed`
6. `[M1a-2] [ActionFlow] 将内联 ActionList 重构为具名声明式 ActionFlow`
7. `[M1a-3] [Consistency] 建立六个消费面的一致性测试套件`
8. `[M1b-1] [DataSource] 引入 OperationRef、OperationResolver 与 DataSourceExecutor`
9. `[M1b-2] [DataSource] 建立查询资源状态机及 Table/Form 标准绑定`
10. `[M1.5-1] [Storage] 页面、快照与版本存储数据库化`
11. `[M1.5-2] [Self-Healing] 运行时错误采集与 Agent 修复建议闭环`
12. `[M1.5-3] [UX] 画布原位 Prompt 和 Patch 空间级差异预览`
13. `[M2-1] [Catalog] 建立 API Catalog 领域模型与 OpenAPI/YApi 导入器`
14. `[M2-2] [Security] 建立 destructive 操作控制流校验与一次性确认令牌`
15. `[M2-3] [MCP] 建立受控 MCP Client 与低代码 MCP Server`
16. `[M3-1] [Workflow] 基于数据库的长任务状态机`
17. `[M4-1] [Visual] 基于 Playwright/DOM 的布局和 a11y 规则检查`
18. `[M5-1] [Governance] Design Tokens、业务字典与团队规范版本化管理`
