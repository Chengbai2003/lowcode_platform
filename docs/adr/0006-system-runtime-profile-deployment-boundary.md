...
| 状态         | 新页面/新快照 | 历史快照的预览与编译                     |
| ------------ | ------------- | ---------------------------------------- |
| `active`     | 允许绑定      | 精确三元组匹配时允许                     |
| `deprecated` | 禁止新绑定    | 精确三元组匹配时仍允许                   |
| `disabled`   | 禁止新绑定    | 拒绝预览、执行和编译；仅可原始 JSON 只读 |
...
2. 由不可变 Snapshot 的 `RuntimeCompatibility` 精确匹配历史 Profile，用于恢复、渲染和编译；不可回退到最新 Profile。
